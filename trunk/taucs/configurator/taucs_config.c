...
void emit_makefile(char* configuration_name)
{
  ...
  */
  ...
  for (i=0; files[i].name; i++) {
    ...
    if (files[i].flags & mexsource) {
      fprintf(f, "\t- $(MEX) -c $(MEXFLAGS) $(STDDEFS) $(STDINCS) \\\n");
      fprintf(f, "\t  %s\\\n",get_full_name(i,0));
      fprintf(f, "\t  $(MEXOUTDIRFLG) $(DIROBJ)\n");
    }
  ...
  if ( (files[i].flags & mex) ) {
	...
	fprintf(f, " %s",get_full_name(k,0));
      ...
      fprintf(f,"\t$(MEX) $(MEXLINKFLAGS) \\\n");
      fprintf(f,"\t$(MEXOUTFLG) %s \\\n",get_full_name(i,0));
      fprintf(f, "\t$(DIRMATLABSRC)dummy.c \\\n");
      for (k=0; files[k].name; k++) {
	if (files[k].state != i) continue;
	fprintf(f, "\t%s \\\n",get_full_name(k,object));
      }

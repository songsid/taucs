...
#undef _OLD_RECURSIVE
//#define  _OLD_RECURSIVE
#ifdef _OLD_RECURSIVE
     ...
     /* divides the vertices into different groups (divides the tree is subtrees) */
{
  ...
  do {
	...
	if (first_child[v] != -1) {
	  stack_array1[stack_top] = v;
	  stack_array2[stack_top++] = r;
	  r = v;
	  v=first_child[r];
	  sub_tree_sizes[r] = 1;
	}
	...
	v = next_child[v];
      ...
      if (stack_top > 0) {
	v = stack_array1[--stack_top];
	r = stack_array2[stack_top];
  ...
  } while ((stack_top >0 ) || (v != -1));

  taucs_free(stack_array1);
  taucs_free(stack_array2);
...
#ifdef _OLD_RECURSIVE
static void DFS_visit(graph *precond,int r,byte *color,linked l,int *pi,int *visited,int n)
{
  edge *p;
  int r1;
  ...
  while (p != NULL)
    {
      ...
      if (color[r1]==0)
	{
	  DFS_visit(precond,r1,color,l,pi,visited,n);
	  pi[r1] = r;
	}
      p = p->next;
    }
...
void DFS_visit(graph *precond,int r,byte *color,linked l,int *pi,int *visited, int n)
{
  edge *p;
  int r1;
  int stack_top = 0;
  edge ** stack_array1 = (edge**)taucs_calloc(n+1,sizeof(edge*));
  ...
  do {
	...
	//	taucs_printf("First Visiting %d\n",r);
      ...
      if (stack_top > 0) {
        p = stack_array1[stack_top-1];
        r = stack_array2[stack_top-1];
        stack_top--;
      }
    }
  } while ((stack_top > 0) || (p!=NULL));

  taucs_free(stack_array1);
  taucs_free(stack_array2);
  //taucs_printf("Finished Normally DFS_VISIT\n");
     ...
     */
{
  ...
  */
  ...
  for (r=0; r<n; r++) {
    ...
    roots[rcount++] = r;
    pi[r] = -1;
    DFS_visit(precond,r,color,*l,pi,&visited,n);
  }
				 ...
				 int force_amwb)
{
  ...
  if ((diagnostics & TAUCS_SYM_POS_OFFDIAGONALS) || (force_amwb)) {
    ret =  amwb_preconditioner_create(G_A, diag, rnd, subgraphs);
    return ret;
  }
  else

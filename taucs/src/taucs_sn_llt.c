  ...
  for (child = first_child[sn]; child != -1; child = next_child[child]) {
    ...
    */
    ...
    extend_add_inlet(spawn recursive_multifrontal_supernodal_factor_llt(child,
									...
									A,snL,fail));
#else
    ret_matrix = recursive_multifrontal_supernodal_factor_llt(child,
							      FALSE,
							      bitmaps,
							      A,snL,fail);
    if (!is_root)
      ...
      /* Gil fixed a bug 21/12/2005: snL->sn_up_size[sn] was read, this   */
      /* element was not allocated (and it is not used in the subroutine).*/
        extend_add_wrapper(ret_matrix,
			   &my_matrix,
			   is_root,
			   NULL,
		...
		int* l_nnz)
{
  ...
  if (l_colcount || l_rowcount || l_nnz) {
    ...
    prev_nbr         = (int*)taucs_malloc(n     * sizeof(int));
    first_descendant = (int*)taucs_malloc(n     * sizeof(int));
#endif
